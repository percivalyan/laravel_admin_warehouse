        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <a href="{{ route('barang_masuks.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded">
                            ...
                            @foreach ($barangMasuks as $barangMasuk)
                                <tr>
                                    ...
                                    <td class="border px-4 py-2">
                                        <a href="{{ route('barang_masuks.show', $barangMasuk) }}" class="bg-green-500 text-white px-2 py-1 rounded">{{ __('Show') }}</a>
                                        <a href="{{ route('barang_masuks.edit', $barangMasuk) }}" class="bg-yellow-500 text-white px-2 py-1 rounded">{{ __('Edit') }}</a>
                                        <form action="{{ route('barang_masuks.destroy', $barangMasuk) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
